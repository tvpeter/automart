...
const loc = _path2.default.resolve('./');
...
describe('Cars', () => {
  ...
  describe('Create Ad', () => {
    it('should return error 400 if request does not contain all required fields', done => {
      ...
      _chai2.default.request(_index2.default).post(adUrl).type('form').set('x-auth', token).attach('img', _path2.default.join(loc, '/server/test/bmwx6d.jpg')).set('Content-Type', 'image/jpeg').field('status', 'available').field('price', '').field('state', 'new').field('model', 'E350').field('manufacturer', 'BMW').field('body_type', 'car').field('description', 'This is additional description').end((err, res) => {
    ...
    it('should return error 400 if user has the same car that is available', done => {
      ...
      const data = _carsData2.default[0];
      carsArray();
      _chai2.default.request(_index2.default).post(adUrl).type('form').set('x-auth', token).attach('img', _path2.default.join(loc, '/server/test/bmwx6d.jpg')).field('owner', data.owner).field('price', data.price).field('state', data.state).field('status', data.status).field('model', data.model).field('manufacturer', data.manufacturer).field('body_type', data.body_type).field('description', 'This is additional description').end((err, res) => {
    ...
    it('should return error 400 if there is no image', done => {
      ...
      _chai2.default.request(_index2.default).post(adUrl).set('x-auth', token).send(data).end((err, res) => {
        expect(res.body.message).to.eq('Fill all required fields');
        expect(res.status).to.eq(400);
        done();
      });
    ...
    it('should return error 401 if token is not provided', done => {
      ...
      _chai2.default.request(_index2.default).post(adUrl).send(data).end((err, res) => {
  ...
  describe('view available cars by manufacturer', () => {
    const manufacturers = ['BMW', 'TOYOTA', 'FIAT'];
    it('should return all unsold cars by a manufacturer', done => {
      carsArray();
      _chai2.default.request(_index2.default).get(`/api/v1/car/manufacturer/${manufacturers[0]}`).end((err, res) => {
    ...
    it('should return a custom error if no vehicle is found for the manufacturer', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/manufacturer/tyonum').end((err, res) => {
  ...
  describe('view available cars by body type', () => {
    const bodyType = ['SUV', 'SEDAN', 'JEEP', 'PICKUP', 'VAN', 'WAGON', 'CONVERTIBLE', 'HATCHBACK'];
    it('should return all unsold cars by body type', done => {
      carsArray();

      _chai2.default.request(_index2.default).get(`/api/v1/car/bodytype/${bodyType[1]}`).end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('Array');
        done();
      });
    ...
    it('should return error 404 if cars of given body type are not found', done => {
      carsArray();
      _chai2.default.request(_index2.default).get(`/api/v1/car/bodytype/${bodyType[2]}`).end((err, res) => {
        expect(res.status).to.eq(404);
  ...
  describe('view available cars by state', () => {
    const state = ['USED', 'New'];
    it('should return all available cars by state -used', done => {
      carsArray();
      _chai2.default.request(_index2.default).get(`/api/v1/car/state/${state[0]}`).end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
    ...
    it('should return all available cars by state -new', done => {
      carsArray();
      _chai2.default.request(_index2.default).get(`/api/v1/car/state/${state[1]}`).end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
    ...
    it('should return error 404 if cars are not found for selected state -old', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/state/old').end((err, res) => {
        expect(res.status).to.eq(404);
  ...
  describe('view all available cars', () => {
    it('should return all unsold cars', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/cars/status/available').end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
        done();
      });
  ...
  describe('Get ad by id', () => {
    it('should return a single ad details', done => {
      ...
      _chai2.default.request(_index2.default).get(`/api/v1/car/${id}`).end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('should return error 400 with custom message if supplied id is not valid', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/12345678901').end((err, res) => {
        expect(res.status).to.eq(400);
    ...
    it('should return error 404 with custom message if ad is not found', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/9293837414384').end((err, res) => {
        expect(res.status).to.eq(404);
  ...
  describe('Seller update ad price', () => {
    it('should return the ad with updated price', async () => {
      const user = _usersData2.default[0];
      user.isAdmin = false;
      const token = await (0, _generateToken2.default)(user.id, user.isAdmin);
      _carsData2.default[0].owner = user.id;
      const reqData = {
      ...
      const res = await _chai2.default.request(_index2.default).patch(`/api/v1/car/${reqData.adId}`).set('x-auth', token).send(reqData);
    ...
    it('should return error 404 if ad is not found', () => {
      ...
      _chai2.default.request(_index2.default).patch(`/api/v1/car/${reqData.adId}`).set('x-auth', token).send(reqData).end((err, res) => {
        expect(res.status).to.eq(404);
    ...
    it('should return error 401 if another user attempts update an ad', () => {
      ...
      user.isAdmin = false;
      const token = (0, _generateToken2.default)(user.id, user.isAdmin);
      const price = _carsData2.default[0].price - 1000000;
    ...
    it('should return error 401 if user is not logged in', () => {
      ...
      _chai2.default.request(_index2.default).patch(`/api/v1/car/${reqData.adId}`).send(reqData).end((err, res) => {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
      });
    });
  ...
  describe('User can view single ad', () => {
    ...
    it('should return error 404 if ad is not found', done => {
      carsArray();
      const id = _carsData2.default[0].id + 1;
      _chai2.default.request(_index2.default).get(`/api/v1/car/${id}`).end((err, res) => {
        expect(res.status).to.eq(404);
        expect(res.body.message).to.eq('The ad you are looking for is no longer available');
        done();
    ...
    it('should return error 400 if invalid ad id is supplied', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/155873165645').end((err, res) => {
        expect(res.status).to.eq(400);
        expect(res.body.message).to.eq('Invalid ad id');
        done();
      });
  ...
  describe('Get ads within a price range', () => {
    it('should return an array of ads within a price range', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/price/?min=5000000&max=8000000').end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('Minimum should default to 0 if not supplied', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/price/?max=8000000').end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('ARRAY');
        done();
      });
    ...
    it('Maximum should default to 24000000 if not supplied', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/price/?min=2000000').end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('ARRAY');
        done();
      });
    ...
    it('Should return error 404 if no ads are found in the given range', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car/price/?min=12000000&max=24000000').end((err, res) => {
        expect(res.status).to.eq(404);
  ...
  describe('admin view all ads', () => {
    it('should return all ads', done => {
      ...
      carsArray();
      const token = (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).get('/api/v1/car').set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('Array');
    ...
    it('should return error 401 if user is not logged in', done => {
      carsArray();
      _chai2.default.request(_index2.default).get('/api/v1/car').end((err, res) => {
  ...
  describe('Admin can delete a posted ad', () => {
    it('should delete a posted ad', done => {
      ...
      carsArray();
      const token = (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).delete(`/api/v1/car/${_carsData2.default[0].id}`).set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('should return error 401 if user is not admin or not logged in', done => {
      carsArray();
      _chai2.default.request(_index2.default).delete(`/api/v1/car/${_carsData2.default[0].id}`).end((err, res) => {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
        done();
      });
    ...
    it('should return error 404 if wrong ad id is given', done => {
      ...
      const token = (0, _generateToken2.default)(user.id, user.isAdmin);
      const id = _carsData2.default[0].id + 1;
      _chai2.default.request(_index2.default).delete(`/api/v1/car/${id}`).set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(404);
