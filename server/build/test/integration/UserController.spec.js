...
describe('User', () => {
  ...
  describe('User create', () => {
    it('should return a new user with the supplied properties', done => {
      ...
      _chai2.default.request(_index2.default).post(signupUrl).send(userDetails).end((err, res) => {
    ...
    it('should return error if password and its confirmation does not match', done => {
      ...
      _chai2.default.request(_index2.default).post(signupUrl).send(data).end((err, res) => {
  ...
  describe('User Signin', () => {
    it('should return error 400 if user did not supply password', done => {
      usersArray();
      _chai2.default.request(_index2.default).post(loginUrl).send({ email: 'johndoe@google.dev' }).then(res => {
        expect(res.status).to.eq(400);
    ...
    it('should return error 404 if user email is not found', done => {
      ...
      _chai2.default.request(_index2.default).post(loginUrl).send(data).then(res => {
    ...
    it('should return error 401 if password is incorrect for given email', done => {
      ...
      _chai2.default.request(_index2.default).post(loginUrl).send(data).end((err, res) => {
  ...
  describe('User change password', () => {
    it('should return user with updated password', done => {
      ...
      _chai2.default.request(_index2.default).patch('/api/v1/user').set('x-auth', token).send({ currentPassword: 'password', newPassword: 'newpassword' }).end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('should return 400 if current password is wrong', done => {
      ...
      user.isAdmin = false;
      const token = (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).patch('/api/v1/user').set('x-auth', token).send({ currentPassword: 'password1', newPassword: 'anotherpassword' }).end((err, res) => {
    ...
    it('should return 400 if current password is not supplied', async () => {
      ...
      _chai2.default.request(_index2.default).patch('/api/v1/user').set('x-auth', token).send({ newPassword: 'newpassword' }).then(res => {
        expect(res.status).to.eq(400);
    ...
    it('should return 401 if user is not logged in', done => {
      usersArray();
      _chai2.default.request(_index2.default).patch('/api/v1/user').send({ currentPassword: 'password', newPassword: 'newpassword' }).end((err, res) => {
        expect(res.status).to.eq(401);
  ...
  describe('get all users', () => {
    it('should return all registered users', async () => {
      usersArray();
      const user = _usersData2.default[1];
      user.isAdmin = true;
      const token = await (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).get('/api/v1/users').set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('should return error 401 if user is not logged in', () => {
      usersArray();
      _chai2.default.request(_index2.default).get('/api/v1/users').end((err, res) => {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
      });
    });
  ...
  describe('Admin make user admin', () => {
    it('Should make a user an admin', async () => {
      ...
      const newAdmin = _usersData2.default[0];
      _usersData2.default[2].isAdmin = true;
      const user = _usersData2.default[2];

      const token = await (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).patch(`/api/v1/user/${newAdmin.id}`).set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(200);
    ...
    it('Should error 401 if admin is not logged in', () => {
      ...
      _chai2.default.request(_index2.default).patch(`/api/v1/user/${newAdmin.id}`).end((err, res) => {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
      });
    });
    it('Should return error 412 if user is not found', async () => {
      ...
      const token = await (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).patch('/api/v1/user/1212121212121').set('x-auth', token).end((err, res) => {
  ...
  describe('User logout', () => {
    it('should log a user out of the app', async () => {
      ...
      const user = _usersData2.default[0];
      const token = await (0, _generateToken2.default)(user.id, user.isAdmin);
      _chai2.default.request(_index2.default).get('/api/v1/auth/logout').set('x-auth', token).end((err, res) => {
        expect(res.status).to.eq(200);
        // eslint-disable-next-line no-unused-expressions
